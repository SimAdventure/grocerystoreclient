...
const CartPage = () => {
    ...
    const {products, user, axios, currency, cartItems, removeCartItem, getCartCount, getCartAmount ,updateCartItem, navigate, setCartItems } = useAppContext();

    const [cartArray, setCartArray] = useState([]);
    const [address, setAddress] = useState();
    const [selectedAddress, setSelectedAddress] = useState(null);
    const [paymentOptions, setPaymentOption] = useState("COD");
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {/* INDEX IN THE ADDRESS MAP LIST*/}
                                {address.map((address, ) =>(
                                    <p onClick={() => {setSelectedAddress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                                        {address.street}, {address.city}, {address.state}, {address.country}
                                    </p>
                                ))}
                                <p onClick={() => navigate("/add-address")} className="text-primary text-center cursor-pointer p-2 hover:bg-primary">
                                    Add address
                                </p>
                            </div>
                        )}
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    <p className="flex justify-between">
                        <span>Price</span><span>{currency}{getCartAmount() || 0}</span>
                    </p>
                    <p className="flex justify-between">
